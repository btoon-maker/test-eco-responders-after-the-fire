<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title>Eco-Responders Lesson</title>
  <style>
    :root{
      --page:#1f3b2c;
      --page2:#1a3226;
      --paper:#ffffff;
      --paper2:#f8fafc;
      --ink:#0f172a;
      --muted:#475569;
      --border:rgba(15,23,42,0.14);
      --shadow: 0 12px 30px rgba(15, 23, 42, 0.18);
      --radius: 14px;

      --accent:#0ea5a4;
      --accent2:#b45309;
      --danger:#dc2626;
      --warn:#f59e0b;

      --font-body: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif;
    }

    html,body{height:100%;}
    body{
      margin:0;
      font-family: var(--font-body);
      color: var(--ink);
      background:
        radial-gradient(1200px 800px at 20% 10%, rgba(255,255,255,0.06), transparent 55%),
        radial-gradient(900px 700px at 80% 35%, rgba(255,255,255,0.05), transparent 60%),
        linear-gradient(180deg, var(--page), var(--page2));
      background-attachment: fixed;
    }

    header{
      position: sticky;
      top: 0;
      z-index: 50;
      background: rgba(255,255,255,0.92);
      backdrop-filter: blur(8px);
      border-bottom: 1px solid var(--border);
      padding: 16px 18px;
    }

    .header-inner{
      max-width: 980px;
      margin: 0 auto;
      display:flex;
      gap:12px;
      align-items:center;
      justify-content:space-between;
      padding-right: 260px;
      box-sizing: border-box;
    }

    .title{
      font-size: 20px;
      font-weight: 900;
      margin:0;
      color: var(--ink);
    }
    .subtitle{
      margin: 4px 0 0 0;
      color: var(--muted);
      font-size: 13px;
    }

    main{
      max-width: 980px;
      margin: 18px auto 150px auto;
      padding: 0 18px;
      box-sizing: border-box;
    }

    /* Shows only if JS fails */
    .app-error{
      display:none;
      margin: 14px 0;
      border: 1px solid rgba(220,38,38,0.35);
      background: rgba(220,38,38,0.10);
      color: var(--ink);
      border-radius: 14px;
      padding: 12px 14px;
      line-height: 1.45;
      white-space: pre-wrap;
      box-shadow: 0 10px 24px rgba(15,23,42,0.14);
    }
    .app-error.show{ display:block; }
    .app-error strong{ color: rgba(220,38,38,0.95); }

    .card{
      border: 1px solid var(--border);
      border-radius: var(--radius);
      box-shadow: var(--shadow);
      overflow:hidden;
      background: var(--paper);
    }
    .section{ margin: 16px 0; }

    .section-head{
      padding: 14px 16px;
      background: var(--paper2);
      border-bottom: 1px solid var(--border);
      display:flex;
      align-items:center;
      justify-content:space-between;
      gap:12px;
    }
    .section-title{
      font-size: 18px;
      font-weight: 900;
      margin:0;
      color: var(--ink);
    }
    .section-body{ padding: 14px 16px 16px 16px; }

    .content{
      color: var(--ink);
      line-height: 1.6;
      font-size: 15px;
      white-space: pre-wrap;
    }

    .two-col{
      display:flex;
      gap:16px;
      align-items: stretch;
    }
    .two-col .col-text{ flex: 1 1 auto; display:flex; }
    .two-col .col-text > .content{ flex: 1 1 auto; width: 100%; }
    .two-col .col-media{ flex: 0 0 280px; }

    .center-media{ margin-top: 12px; display:flex; justify-content:center; }
    .center-media img{
      max-width: 560px;
      width: 100%;
      border-radius: 12px;
      border: 1px solid var(--border);
      background: #fff;
    }
    .center-media .img-placeholder{ max-width: 560px; }

    .two-col .col-media img{
      width: 100%;
      height: auto;
      border-radius: 12px;
      border: 1px solid var(--border);
      background: #fff;
      display:block;
    }

    .img-placeholder{
      width: 100%;
      min-height: 180px;
      border-radius: 12px;
      border: 1px dashed rgba(15,23,42,0.25);
      background: rgba(15,23,42,0.03);
      display:flex;
      align-items:center;
      justify-content:center;
      text-align:center;
      color: var(--muted);
      padding: 14px;
      box-sizing:border-box;
      font-size: 13px;
      white-space: pre-wrap;
    }

    details.dropdown{
      margin-top: 14px;
      border: 1px solid var(--border);
      border-radius: 12px;
      background: rgba(15,23,42,0.02);
      overflow:hidden;
    }
    details.dropdown summary{
      cursor:pointer;
      padding: 12px 14px;
      font-weight: 900;
      color: var(--accent2);
      list-style:none;
      user-select:none;
      display:inline-flex;
      align-items:center;
      justify-content:flex-start;
      gap:8px;
    }
    details.dropdown summary::-webkit-details-marker{display:none;}
    details.dropdown summary::after{
      content: "▸";
      color: rgba(15,23,42,0.60);
      font-weight: 900;
      transform: translateY(-1px);
      transition: transform 0.15s ease;
    }
    details.dropdown[open] summary::after{ content: "▾"; }
    .dropdown-body{
      padding: 0 14px 14px 14px;
      color: var(--ink);
      white-space: pre-wrap;
      line-height: 1.6;
      font-size: 14px;
    }

    .choice-block{
      margin-top: 14px;
      padding: 12px 12px;
      border-radius: 12px;
      border: 1px solid rgba(14,165,164,0.25);
      background: rgba(14,165,164,0.06);
    }
    .choice-title{
      font-weight: 900;
      margin: 0 0 8px 0;
      white-space: pre-wrap;
      color: var(--ink);
    }

    .btn-row{
      display:flex;
      flex-wrap:wrap;
      gap:10px;
      margin-top: 10px;
    }

    button.btn{
      appearance:none;
      border: 1px solid rgba(15,23,42,0.18);
      background: #fff;
      color: var(--ink);
      padding: 10px 12px;
      border-radius: 12px;
      cursor:pointer;
      font-weight: 900;
      font-size: 14px;
      transition: transform 0.05s ease, background 0.2s ease, border-color 0.2s ease;
      box-shadow: 0 6px 14px rgba(15, 23, 42, 0.06);
    }
    button.btn:hover{
      background: rgba(15,23,42,0.02);
      border-color: rgba(15,23,42,0.26);
    }
    button.btn:active{ transform: translateY(1px); }
    button.btn.primary{
      background: rgba(14,165,164,0.12);
      border-color: rgba(14,165,164,0.45);
    }
    button.btn.primary:hover{ background: rgba(14,165,164,0.16); }
    button.btn.warning{
      background: rgba(245,158,11,0.14);
      border-color: rgba(245,158,11,0.45);
    }
    button.btn.danger{
      background: rgba(220,38,38,0.10);
      border-color: rgba(220,38,38,0.45);
    }
    button.btn[disabled]{ opacity:0.6; cursor:not-allowed; }

    .feedback{
      margin-top: 12px;
      padding: 12px;
      border-radius: 12px;
      border: 1px solid rgba(245,158,11,0.45);
      background: rgba(245,158,11,0.12);
      white-space: pre-wrap;
      line-height: 1.6;
      color: var(--ink);
    }

    .journal{
      margin-top: 14px;
      padding: 12px 12px;
      border-radius: 12px;
      border: 1px solid rgba(15,23,42,0.14);
      background: rgba(15,23,42,0.02);
    }
    .journal h4{
      margin:0 0 8px 0;
      font-size: 16px;
      font-weight: 900;
      color: var(--accent);
      white-space: pre-wrap;
    }
    .journal .prompt{
      white-space: pre-wrap;
      line-height: 1.6;
      color: var(--ink);
      margin: 6px 0 10px 0;
      font-size: 14px;
    }
    .journal textarea{
      width: 100%;
      min-height: 120px;
      resize: vertical;
      border-radius: 12px;
      border: 1px solid rgba(15,23,42,0.18);
      background: #fff;
      color: var(--ink);
      padding: 10px 10px;
      box-sizing:border-box;
      font-size: 14px;
      line-height: 1.5;
      outline:none;
    }
    .journal textarea:focus{
      border-color: rgba(14,165,164,0.60);
      box-shadow: 0 0 0 3px rgba(14,165,164,0.15);
    }
    .journal .mission{
      margin-top: 12px;
      padding-top: 10px;
      border-top: 1px dashed rgba(15,23,42,0.20);
    }

    .tag{
      display:inline-block;
      padding: 3px 8px;
      border-radius: 999px;
      font-size: 12px;
      font-weight: 900;
      border: 1px solid rgba(15,23,42,0.18);
      background: #fff;
      color: var(--muted);
    }

    .resume-widget{
      position: fixed;
      top: 10px;
      right: 10px;
      z-index: 1000;
      width: 260px;
      background: rgba(255,255,255,0.92);
      backdrop-filter: blur(8px);
      border: 1px solid var(--border);
      border-radius: 14px;
      box-shadow: var(--shadow);
      overflow:hidden;
      font-size: 12px;
    }
    .resume-widget .rw-body{ padding: 10px; }
    .rw-row{ display:flex; gap:8px; align-items:center; flex-wrap:wrap; }
    .rw-row input{
      flex: 1 1 auto;
      min-width: 120px;
      border-radius: 10px;
      border: 1px solid rgba(15,23,42,0.18);
      background: #fff;
      color: var(--ink);
      padding: 8px 8px;
      font-weight: 900;
      font-size: 12px;
      outline:none;
    }
    .rw-note{
      margin-top: 8px;
      color: var(--muted);
      line-height: 1.35;
    }

    .modal-backdrop{
      position: fixed;
      inset: 0;
      background: rgba(2,6,23,0.40);
      z-index: 2000;
      display:none;
      align-items:center;
      justify-content:center;
      padding: 18px;
    }
    .modal-backdrop.show{ display:flex; }
    .modal{
      width: min(520px, 100%);
      background: #fff;
      border: 1px solid var(--border);
      border-radius: 16px;
      box-shadow: 0 18px 60px rgba(15,23,42,0.20);
      overflow:hidden;
    }
    .modal-head{
      padding: 12px 14px;
      border-bottom: 1px solid var(--border);
      display:flex;
      align-items:center;
      justify-content:space-between;
      gap:10px;
      background: var(--paper2);
    }
    .modal-head strong{ font-size: 14px; color: var(--ink); }
    .modal-body{ padding: 14px; color: var(--ink); }
    .small-label{ color: var(--muted); margin: 10px 0 6px 0; font-weight: 900; }
    .codebox{ display:flex; gap:6px; align-items:center; }
    input.code{
      width: 100%;
      border-radius: 10px;
      border: 1px solid rgba(15,23,42,0.18);
      background: #fff;
      color: var(--ink);
      padding: 8px 8px;
      font-weight: 900;
      font-size: 12px;
      outline:none;
    }
    .helper{
      margin-top: 10px;
      padding: 10px 12px;
      border-radius: 12px;
      border: 1px solid rgba(14,165,164,0.25);
      background: rgba(14,165,164,0.06);
      color: var(--ink);
      line-height: 1.45;
      font-size: 12px;
    }

    .export-bar{
      position: fixed;
      left: 0;
      right: 0;
      bottom: 0;
      z-index: 900;
      padding: 10px 12px;
      background: rgba(31,59,44,0.85);
      backdrop-filter: blur(8px);
      border-top: 1px solid rgba(255,255,255,0.14);
      display:flex;
      justify-content:center;
    }
    .export-inner{
      width: min(980px, 100%);
      display:flex;
      justify-content:flex-end;
      gap:10px;
      padding: 0 6px;
      box-sizing:border-box;
    }

    .note{
      color: rgba(255,255,255,0.82);
      font-size: 12px;
      margin-top: 10px;
      line-height: 1.35;
    }

    /* Feedback Loop (same as before) */
    .journal-card{
      border:1px solid rgba(15,23,42,0.14);
      border-radius:16px;
      padding:18px;
      background: rgba(15,23,42,0.02);
      margin: 18px 0;
    }
    .journal-title{ margin:0 0 6px 0; font-size: 1.35rem; color: var(--ink); }
    .hl-yellow{ color: var(--accent2); font-weight: 900; }

    .loop-layout{
      display:grid;
      grid-template-columns: 320px 1fr;
      gap:16px;
      align-items:start;
    }

    @media (max-width: 980px){
      .loop-layout{ grid-template-columns: 1fr; }
      .header-inner{ padding-right: 0; }
      .resume-widget{ position: static; width: auto; margin: 10px 18px 0 18px; }
    }

    .word-bank{
      border:1px solid rgba(15,23,42,0.14);
      border-radius:14px;
      padding:14px;
      background: #fff;
    }
    .word-bank h3{ margin:0 0 10px 0; font-size: 1.05rem; color: var(--accent2); }
    .chip-bank{ display:flex; flex-wrap:wrap; gap:10px; }
    .chip{
      border:1px solid rgba(15,23,42,0.16);
      background: rgba(15,23,42,0.02);
      color: var(--ink);
      border-radius:999px;
      padding:8px 10px;
      cursor:grab;
      user-select:none;
      font-weight:700;
      font-size:.92rem;
    }
    .chip:active{ cursor:grabbing; }
    .bank-actions{ margin-top:12px; display:flex; gap:10px; }
    .hint{ margin:10px 0 0 0; color: var(--muted); font-size:.9rem; }

    .loop-canvas{
      border:1px solid rgba(15,23,42,0.14);
      border-radius:14px;
      padding:14px;
      background: #fff;
      min-height: 500px;
      overflow: visible;
      position: relative;
      box-sizing: border-box;
    }

    .dropzone{
      width: 220px;
      max-width: 220px;
      border:1px dashed rgba(15,23,42,0.25);
      border-radius:14px;
      padding:10px 12px;
      background: rgba(15,23,42,0.02);
      box-sizing: border-box;
    }
    .dz-label{
      display:block;
      font-size:.85rem;
      color: var(--muted);
      margin-bottom:6px;
      font-weight: 800;
    }
    .dz-value{
      min-height: 38px;
      display:flex;
      align-items:center;
      gap:10px;
    }
    .dz-value:empty::before{
      content: attr(data-empty);
      color: rgba(71,85,105,0.8);
    }
    .dropzone.dragover{
      border-color: rgba(14,165,164,0.55);
      background: rgba(14,165,164,0.08);
    }

    .placed-chip{
      display:inline-flex;
      align-items:center;
      gap:8px;
      border:1px solid rgba(15,23,42,0.16);
      background: #fff;
      padding:8px 10px;
      border-radius:999px;
      cursor:grab;
      font-weight:900;
      color: var(--ink);
      max-width: 100%;
      flex-wrap: wrap;
      white-space: normal;
      line-height: 1.2;
    }
    .remove-x{
      color: var(--muted);
      font-weight:900;
      border:1px solid rgba(15,23,42,0.16);
      border-radius:999px;
      width:22px;
      height:22px;
      display:inline-flex;
      align-items:center;
      justify-content:center;
      background: rgba(15,23,42,0.02);
      flex: 0 0 auto;
    }

    .loop-example, .science-prompt, .loop-flow{
      margin-top:12px;
      padding:10px 12px;
      border-radius:12px;
      background: rgba(15,23,42,0.02);
      border:1px solid rgba(15,23,42,0.14);
      font-size:.95rem;
      width: 100%;
      box-sizing: border-box;
      color: var(--ink);
    }

    .loop-ring{
      position: relative;
      width: 100%;
      height: 400px;
      min-height: 400px;
      margin-top: 6px;
      padding: 12px;
      box-sizing: border-box;
    }
    .loop-ring svg{
      position:absolute;
      inset: 0;
      width: 100%;
      height: 100%;
      pointer-events:none;
      overflow: visible;
    }

    .dz-top{
      position:absolute;
      left:50%;
      top: 6px;
      transform: translateX(-50%);
    }
    .dz-right{
      position:absolute;
      right: 22px;
      top:50%;
      transform: translateY(-50%);
    }
    .dz-bottom{
      position:absolute;
      left:50%;
      bottom: 6px;
      transform: translateX(-50%);
    }
    .dz-left{
      position:absolute;
      left: 22px;
      top:50%;
      transform: translateY(-50%);
    }

    @media (max-width: 980px){
      .loop-ring{ height: 520px; min-height: 520px; padding: 0; }
      .dropzone{ width: 100%; max-width: 100%; }
      .dz-top, .dz-right, .dz-bottom, .dz-left{
        position: static;
        transform: none;
        margin-top: 10px;
      }
      .loop-ring svg{ display:none; }
    }

    .reflection-grid{
      display:grid;
      grid-template-columns: 1fr 1fr;
      gap:12px;
      margin-top:14px;
    }
    @media (max-width: 980px){
      .reflection-grid{ grid-template-columns:1fr; }
    }

    .reflection-card{
      border:1px solid rgba(15,23,42,0.14);
      border-radius:14px;
      padding:12px;
      background: rgba(15,23,42,0.02);
    }
    .reflection-card h3{ margin:0 0 8px 0; font-size:1rem; color: var(--ink); }
    .reflection-card textarea{
      width:100%;
      border-radius:12px;
      border:1px solid rgba(15,23,42,0.18);
      background: #fff;
      color: var(--ink);
      padding:10px;
      resize: vertical;
      box-sizing: border-box;
      outline:none;
    }
    .reflection-card textarea:focus{
      border-color: rgba(14,165,164,0.60);
      box-shadow: 0 0 0 3px rgba(14,165,164,0.15);
    }

    .journal-actions{
      display:flex;
      align-items:center;
      gap:12px;
      margin-top:12px;
    }
    .save-status{ color: var(--muted); font-size:.92rem; }
  </style>
</head>
<body>
  <header>
    <div class="header-inner">
      <div>
        <h1 class="title">Eco-Responders</h1>
        <p class="subtitle">Single-page interactive lesson (sections reveal only when you click).</p>
      </div>
    </div>
  </header>

  <aside class="resume-widget" aria-label="Pause & Resume Later widget">
    <div class="rw-body">
      <div class="rw-row">
        <button class="btn primary" id="pauseBtn" type="button">Pause &amp; Resume Later</button>
      </div>

      <div style="height:10px"></div>

      <div class="rw-row">
        <input id="pasteCode" type="text" placeholder="Paste Resume Code Here" />
        <button class="btn warning" id="resumeBtn" type="button">Resume</button>
      </div>

      <div class="rw-note" id="rwNote">
        Your progress saves automatically on this device. To continue on another device, use “Pause &amp; Resume Later” to copy the Resume Code.
      </div>
    </div>
  </aside>

  <div class="modal-backdrop" id="pauseModalBackdrop" aria-hidden="true">
    <div class="modal" role="dialog" aria-modal="true" aria-label="Pause & Resume Later">
      <div class="modal-head">
        <strong>Pause &amp; Resume Later</strong>
        <button class="btn danger" id="closePauseModal" type="button">Close</button>
      </div>
      <div class="modal-body">
        <div class="small-label">Resume Code</div>
        <div class="codebox">
          <input class="code" id="resumeCode" type="text" readonly value="" />
          <button class="btn" id="copyCodeBtn" type="button">Copy</button>
        </div>

        <div class="helper" id="resumeHelper">
          <strong>How it works:</strong><br/>
          • Your work is saved automatically on <em>this</em> device/browser.<br/>
          • To continue on another device, copy the Resume Code above and paste it into the Resume box on the other device.
        </div>

        <div class="small-label">Status</div>
        <span class="tag" id="saveStatus">Saved</span>
      </div>
    </div>
  </div>

  <main>
    <div id="appError" class="app-error" role="alert"></div>
    <div id="lesson"></div>
    <div class="note" id="imagesNote"></div>
  </main>

  <div class="export-bar">
    <div class="export-inner">
      <button class="btn primary" id="exportBtn" type="button">Export My Journal (PDF)</button>
    </div>
  </div>

  <!-- Load libraries BEFORE app.js -->
  <script defer src="https://cdn.jsdelivr.net/npm/lz-string@1.5.0/libs/lz-string.min.js"></script>
  <script defer src="https://cdn.jsdelivr.net/npm/jspdf@2.5.1/dist/jspdf.umd.min.js"></script>

  <!-- App LAST (also defer) -->
  <script defer src="./app.js"></script>
</body>
</html>
